...
IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_NAME = 'sp_WhoIsActive_debug')
	EXEC ('CREATE PROC dbo.sp_WhoIsActive_debug AS SELECT ''stub version, to be replaced''')
GO
...
License: 
	...
	written consent.
*********************************************************************************************/
ALTER PROC dbo.sp_WhoIsActive_debug
(
--~
...
BEGIN;
	...
	insert into dbo.table_sp_whoisactive 
	(
		ddhhmmssmss				,
		session_id				,
		sql_text				,
	...
	update t
		set t.id_exec =  isnull((select max(id_exec)+1 from dbo.table_sp_whoisactive),0)
	from dbo.table_sp_whoisactive t
	where id_exec is null 
