/* ------------------------------ *\
   ...
   Adaptação para migração de índices - Giovanni Tolosa - i4pro
\* ------------------------------ */
...
DECLARE
	@nm_schema		varchar(30)		= 'cad',					--schema origem
	@nm_table_orig  varchar(300)	= 't_pessoas',				--tabela origem
	@nm_schema_dest varchar(30)     = 'dba',					--schema destino
	@nm_table_dest	varchar(300)	= 'teste_sinonimo_new'		--tabela destino

DECLARE 
	@Match TABLE ([Schema] varchar(0128))

   INSERT @Match ([Schema])
   SELECT S.name
     FROM sys.schemas AS S
    WHERE S.name = @nm_schema	
...
WHILE @@FETCH_STATUS = 0
    ...
    SET @SQLCode = 'if not exists (select 1 from sys.indexes where name ='''+@SQLServerName+ ''' and  object_id = OBJECT_ID('''+ @nm_schema_dest +'.'+ @nm_table_dest+''')) ' + CHAR(10)+
	'BEGIN   ' +CHAR(10)
                
    PRINT @SQLCode

    SET @SQLCode = '	CREATE '
                 + CASE WHEN @index_type IN (1, 2      ) AND @is_unique != 0         THEN 'UNIQUE '                                                ELSE SPACE(01) END
                 + CASE WHEN @index_type IN (   2,    6)                             THEN 'NON'                                                    ELSE SPACE(01) END + 'CLUSTERED '
                 + CASE WHEN @index_type IN (      5, 6)                             THEN 'COLUMNSTORE '                                           ELSE SPACE(01) END + 'INDEX [' + @SQLServerName + '] ON [' + @nm_schema_dest + '].[' + @nm_table_dest + ']'
